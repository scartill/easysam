...
def test_dynamottl_generation():
    example_path = Path('example/dynamottl')
    ...
    # Verify DynTTLMyItem
    assert 'DynTTLMyItem' in resources
    myitem = resources['DynTTLMyItem']
    assert myitem['Type'] == 'AWS::DynamoDB::Table'
    assert myitem['Properties']['TimeToLiveSpecification']['AttributeName'] == 'ExpireAt'
    assert myitem['Properties']['TimeToLiveSpecification']['Enabled'] is True
