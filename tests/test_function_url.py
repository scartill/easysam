...
def test_function_url_generation():
    ...
    assert hello_custom['Properties']['FunctionUrlConfig']['Cors']['AllowMethods'] == ['GET', 'POST']
...
def test_function_url_validation_error():
    example_path = Path("example/functionurl")
    cliparams = {'verbose': True}
    deploy_ctx = {'environment': 'dev', 'target_region': 'us-east-1'}
    ...
    # Modify for error
    resources_data['functions']['hello-world']['function_url'] = {'auth_type': 'INVALID'}
